...
def train(net, pacman_loader, monster_loader, criterion, optimizer, epochs=50):
    net.train()
    running_loss = 0.0
    best_loss = 10
    for epoch in range(epochs):
        ...
        val_loss = val(net, monster_loader, criterion)
        ...
        if val_loss < best_loss:
            ...
            print("Loss improve from %5f to %5f. Save best checkpoint"%(best_loss, val_loss))
            best_loss = val_loss


def val(net, monster_loader, criterion):
    # net.eval()
    val_loss = 0.0
...
if __name__ == '__main__':
    ...
    pacman_set = Pacman(dir='data', pad=False)
    pacman_loader = data.DataLoader(
        ...
        pin_memory=True,
        drop_last=True)
    moster_set = Pacman(dir='data', pad=False, catagory=['monster'], random_template = False)
    ...
    # net.load_state_dict(torch.load('output/checkpoint/model_best.pth')['TemplateMatching'])
